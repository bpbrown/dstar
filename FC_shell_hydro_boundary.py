...
using a Lane-Emden structure and boundary driving (rather than internal heat).
...
grad_s0 = (1/γ*grad_θ0 - (γ-1)/γ*grad_Υ0).evaluate()
...
problem.add_equation((ρ0*ddt(u) # assumes grad_s0 = 0
                      ...
                      +Co2*ρ0_h0_g*(np.expm1(θ)-θ)*grad_s0))
...
if args['--thermal_equilibrium']:
    ...
    if perturbations['s']['g'].size > 0 :
        for i, r_i in enumerate(r[0,0,:]):
            s['g'][:,:,i] += perturbations['s'](r=r_i).evaluate()['g'][0,0,0]
            θ['g'][:,:,i] += perturbations['θ'](r=r_i).evaluate()['g'][0,0,0]
